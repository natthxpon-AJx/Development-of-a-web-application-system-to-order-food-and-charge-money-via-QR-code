...
-- เพิ่มคอลัมน์ sku ใน menu_items ตามคำแนะนำของอาจารย์
--
-- วิธีใช้: เปิด phpMyAdmin (http://localhost/phpmyadmin) แล้วไปที่แท็บ
-- "Import" เลือกไฟล์นี้ แล้วกด Go — จะสร้างฐานข้อมูล chamakmai
-- พร้อมตารางและข้อมูลตัวอย่างให้อัตโนมัติ
-- ================================================================
...
CREATE TABLE IF NOT EXISTS menu_items (
  ...
  sku             VARCHAR(50) UNIQUE,                     -- เพิ่มตามคำแนะนำอาจารย์: รหัสสินค้า (Stock Keeping Unit)
  ...
  options_config  JSON NULL                                -- เช่น {"sweetness":["0%","50%","100%"]}
...
CREATE TABLE IF NOT EXISTS orders (
  ...
  payment_method  VARCHAR(20) NULL,                        -- 'cash' | 'qr_code'
...
CREATE TABLE IF NOT EXISTS order_items (
  ...
  menu_id           INT NULL,
  item_name         VARCHAR(100) NOT NULL,                 -- snapshot ชื่อเมนู ณ เวลาสั่ง
  price_at_order    DECIMAL(10,2) NOT NULL,                 -- snapshot ราคา ณ เวลาสั่ง
  quantity          INT NOT NULL DEFAULT 1,
  selected_options  JSON NULL,                              -- เช่น {"sweetness":"50%"}
...
CREATE TABLE IF NOT EXISTS `tables` (
  table_id          VARCHAR(10) PRIMARY KEY,               -- เช่น T01, T02
...
-- ไม่มี seed users สำหรับ login ใน schema นี้ เพราะระบบ login ต้องใช้ผู้ใช้งานจริงจาก backend
